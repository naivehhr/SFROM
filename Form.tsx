...
import { cleanObj, get, set, merge, isEmpty, cloneDeep, validateFields } from './utils';
...
export default class Form extends React.Component<IFormProps, IFormState> {
  static getDerivedStateFromProps(props: IFormProps, state: IFormState) {
    ...
    return merge(state, { data: formData, error: formError, schema: formSchema });
  }
  formValidates: { idPath: string; validate: object; label: string; type: string }[] = [];
  subInstance: any;
  ...
  handleChange = (params: any) => (value: any) => {
    ...
    this.setState({ data: newData, error: cleanObj(newError) }, () => {
      const { error, data } = this.state;
      onChange &&
        onChange({ formError: cloneDeep(error), formData: cloneDeep(data), idPath, value });
    });
  };
  ...
  validateForm = (): boolean => {
    const { data, schema, error } = this.state;
